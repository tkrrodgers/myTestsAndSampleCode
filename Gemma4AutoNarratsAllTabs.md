...
*An unattended demonstration mode in which Gemma 4 walks all 40 scenes, operates every control, and explains what is happening — with Claude Opus 4.8 as the narration failover.*
...
**Source of truth for counts:** `server/Services/AutopilotManifest.cs` — 152 steps over 40 scenes, 27 of them bridge steps, 4 hard-blocked in-process steps. The scene-by-scene tables in §11 were written against the 34-scene build and have not been extended for scenes 35–40; the manifest has.
...
| **Rehearsed** | All 40 scenes, narration pre-cached, results from the current session | Result narration only | A leadership demo that has been rehearsed |
| **Full** | All 40 scenes, every model stage executed live | **27 bridge steps, several fanning out to multiple models** (§11) | Proving the whole pipeline actually works |
...
| **Speculative decoding launches `llama-server` on the host** (scene 38), **notes-to-domains embeds and trains against a repository on disk** (scene 39) and **LLM in the Room launches `llama-server` and `whisper-server`** (scene 40) | Same `AllowInProcessExecution` hard block as mutation testing. |
...
The complete walkthrough as designed against the **34-scene build**. Scenes 35–40 (Classify Context, Tell LLM to Focus/Ignore, Right LLM for the job, Speculative decoding locally, Notes to domains, LLM in the Room) are specified only in `AutopilotManifest.cs`; their fact packs and `must_not_claim` lists follow the same conventions as the tables below. Every control listed below was extracted from the markup current at the time, not assumed.

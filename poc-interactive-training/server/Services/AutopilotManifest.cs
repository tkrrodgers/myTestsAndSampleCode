...
public static class AutopilotManifest
{
    ...
    [
        ...
        S("s26.01", "regression-adequacy", 26, AutoKind.Observe,
            ["A mutant is one deliberately injected defect, such as flipping a comparison.",
             "A mutant is killed when a test fails as it should; a survivor is a defect the suite would ship."]),
        S("s26.02", "regression-adequacy", 26, AutoKind.Click, selector: "[data-auto='s26.mutate']",
            wait: AutoWait.Element, waitKey: "[data-auto='s26.mutation-result']", timeout: 200, result: true,
            needsExec: true,
            facts: ["The suite is compiled and executed, then one defect is injected at a time and it is run again."]),
        S("s26.03", "regression-adequacy", 26, AutoKind.Click, selector: "[data-auto='s26.guidance']", bridge: true,

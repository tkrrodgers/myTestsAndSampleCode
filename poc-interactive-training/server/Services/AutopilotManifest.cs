    ...
    [
        ...
        S("s01.02", "introduction", 1, AutoKind.Scroll, selector: "[data-auto='s01.gaps']",
            facts: ["Eight capabilities the role calls for are not yet covered by the lab.",
                    "Three of the eight have no scene at all: runtime incident response, the operating model, and whether the training changed behaviour."],
        ...
        // ---- 8 prompt-challenge ---------------------------------------------
        S("s09.01", "prompt-challenge", 8, AutoKind.Observe,
            ["The rubric has six areas and is visible before the review runs."]),
        S("s09.02", "prompt-challenge", 8, AutoKind.Observe,
        ...
        S("s09.03", "prompt-challenge", 8, AutoKind.Click, selector: "[data-auto='s09.submit']", bridge: true,
        ...
        S("s09.04", "prompt-challenge", 8, AutoKind.Observe, result: true,
        ...
        S("s09.05", "prompt-challenge", 8, AutoKind.Observe, result: true,
        ...
        // ---- 9 model-comparison --------------------------------------------
        S("s10.01", "model-comparison", 9, AutoKind.Select, selector: "[data-auto='s10.topic']", value: "gke-autoscaling",
            facts: ["The same question is put to three models."]),
        S("s10.02", "model-comparison", 9, AutoKind.Click, selector: "[data-auto='s10.run']", bridge: true,
        ...
        S("s10.03", "model-comparison", 9, AutoKind.Observe, result: true,
        ...
        S("s10.04", "model-comparison", 9, AutoKind.Observe,
        ...
        // ---- 10 llm-support --------------------------------------------------
        S("s11.01", "llm-support", 10, AutoKind.Observe,
            ["The requirement is a Google Cloud migration brief."]),
        S("s11.02", "llm-support", 10, AutoKind.Click, selector: "[data-auto='s11.run']", bridge: true,
        ...
        S("s11.03", "llm-support", 10, AutoKind.Observe, result: true,
        ...
        // ---- 11 round-trip ---------------------------------------------------
        S("s12.01", "round-trip", 11, AutoKind.Observe,
            ["The original class is the input to Gemma 4."]),
        S("s12.02", "round-trip", 11, AutoKind.Click, selector: "[data-auto='s12.run']", bridge: true,
        ...
        S("s12.03", "round-trip", 11, AutoKind.Observe, result: true,
        ...
        // ---- 12 modernize ----------------------------------------------------
        S("s13.01", "modernize", 12, AutoKind.Click, selector: "[data-auto='s13.run']", bridge: true,
        ...
        S("s13.02", "modernize", 12, AutoKind.Observe, result: true,
        ...
        // ---- 13 audit-code ---------------------------------------------------
        S("s14.01", "audit-code", 13, AutoKind.Click, selector: "[data-auto='s14.run']", bridge: true,
        ...
        S("s14.02", "audit-code", 13, AutoKind.Observe, result: true,
            facts: ["The numeric rating came from static analysis.",
                    "The model supplied the judgement, not the numbers."]),

        // ---- 14 audit-context ------------------------------------------------
        S("s15.01", "audit-context", 14, AutoKind.Click, selector: "[data-auto='s15.run']", bridge: true,
        ...
        S("s15.02", "audit-context", 14, AutoKind.Observe, result: true,
        ...
        // ---- 15 llm-language -------------------------------------------------
        S("s16.01", "llm-language", 15, AutoKind.Observe,
        ...
        S("s16.02", "llm-language", 15, AutoKind.Click, selector: "[data-auto='s16.run']", bridge: true,
        ...
        S("s16.03", "llm-language", 15, AutoKind.Observe, result: true,
            facts: ["The compiled output is a deterministic execution result."]),
        S("s16.04", "llm-language", 15, AutoKind.Click, selector: "[data-auto='s16.benchmark']",
        ...
        // ---- 16 language-test ------------------------------------------------
        S("s17.01", "language-test", 16, AutoKind.Observe,
        ...
        S("s17.02", "language-test", 16, AutoKind.Click, selector: "[data-auto='s17.run']", bridge: true,
        ...
        S("s17.03", "language-test", 16, AutoKind.Observe, result: true,
        ...
        // ---- 17 cobol-migration ----------------------------------------------
        S("s18.01", "cobol-migration", 17, AutoKind.Observe,
            ["The oracle is the real COBOL program, compiled and executed.",
             "Its captured output is the ground truth for this scene."]),
        S("s18.02", "cobol-migration", 17, AutoKind.Click, selector: "[data-auto='s18.run']", bridge: true,
        ...
        S("s18.03", "cobol-migration", 17, AutoKind.Observe, result: true,
            facts: ["Nothing in this scene is graded by a model.",
                    "The verdict is decided by execution against the oracle."]),

        // ---- 18 cobol-domains ------------------------------------------------
        S("dom.01", "cobol-domains", 18, AutoKind.Scroll, selector: "[data-auto='dom.coverage']",
            facts: ["The first thing shown is how much of the code the tool could read: programs, copybooks, database statements and calls.",
                    "A map drawn from part of the code is a claim about that part only, so these numbers come before any conclusion."],
            mustNot: ["that the boundaries shown are ratified"]),
        S("dom.01b", "cobol-domains", 18, AutoKind.Scroll, selector: "[data-auto='dom.jcl']",
            facts: ["The batch job control and the CICS transaction definitions are read as well as the COBOL.",
                    "They say which transaction starts which program, which job step runs the loader, and which tables the job creates.",
                    "Five tables are created but only two are read by any program in the sample."],
            mustNot: ["that the JCL facts are weighted into the grouping"]),
        S("dom.02", "cobol-domains", 18, AutoKind.Click, selector: "[data-auto='dom.pdg']",
            wait: AutoWait.Element, waitKey: "[data-auto='dom.pdg-result']", timeout: 30, result: true,
            facts: ["A statement-level view of one program is built now, in memory, and thrown away afterwards.",
                    "It is never built for the whole estate because it answers questions about statements, not about domains."]),
        S("dom.03", "cobol-domains", 18, AutoKind.Observe, result: true,
            facts: ["The view lists which statements only run under a condition and which fields feed which, each with a line number.",
                    "It states plainly which COBOL constructs it does not yet model."],
            mustNot: ["that this view establishes a domain boundary"]),
        S("dom.04", "cobol-domains", 18, AutoKind.Scroll, selector: "[data-auto='dom.fields']",
            facts: ["Every field in every copybook is rewritten as a plain-English card built by rules from the source: its group, its database column, its type.",
                    "The card is what gets searched, because a raw declaration matches on shape rather than meaning."],
            mustNot: ["that a model wrote the field cards"]),
        S("dom.04b", "cobol-domains", 18, AutoKind.Scroll, selector: "[data-auto='dom.neurons']",
            facts: ["The programs are grouped by how connected they are, and the grouping is re-run at seven settings.",
                    "Programs that stay together at every setting are a stable core; programs that move are referred to a person rather than forced into a group.",
                    "Two groups were found: the vehicle catalogue and the vehicle inventory. Five programs cannot prove a real boundary; this proves the machinery."],
            mustNot: ["that this partition is a result about the real estate"]),
        S("dom.05", "cobol-domains", 18, AutoKind.Click, selector: "[data-auto='dom.cortex']",
            wait: AutoWait.Element, waitKey: "[data-auto='dom.cortex-result']", timeout: 30, result: true,
            facts: ["A question is answered by exact word match, an optional meaning-based match, and a direct look-up in the map, then merged.",
                    "Who reads a table is answered exactly from the map, and every answer cites a file and line."]),
        S("dom.06", "cobol-domains", 18, AutoKind.Scroll, selector: "[data-auto='dom.findings']",
            facts: ["Every finding is produced from the evidence on the page, including the program the sample hands over to but does not contain, and the loader whose tables cannot be named from source.",
                    "The COBOL is read by the standard ANTLR Cobol85 grammar, generated to C# and committed, so the build needs no Java."],
            mustNot: ["that the boundaries shown have been ratified"]),

        // ---- 19 ticket-gate --------------------------------------------------
        // Step-id prefixes below are stable historic identifiers and no longer track the scene number.
        // SceneNumber is authoritative; data-auto tokens must keep matching the markup.
        ...
        // ---- 35 classify-context ---------------------------------------------------------
        S("cc.01", "classify-context", 35, AutoKind.Scroll, selector: "[data-auto='cc.designs']",
            facts: ["Two service designs were written separately and every section carries a sealed domain label: fixed income, crypto, or shared.",
                    "Two fixed-income sections deliberately depend on crypto-labelled sections; those are the traps."]),
        S("cc.02", "classify-context", 35, AutoKind.Scroll, selector: "[data-auto='cc.mixed']",
            facts: ["The sections are shuffled together with a fixed seed and renumbered, standing in for a chat history that drifted across topics.",
                    "The sealed labels are not in the text the classifier or the models see."]),
        S("cc.03", "classify-context", 35, AutoKind.Click, selector: "[data-auto='cc.classify']",
            wait: AutoWait.Element, waitKey: "[data-auto='cc.classify-result']", timeout: 60, result: true,
            facts: ["A local classifier — embeddinggemma vectors through ML.NET, trained on the two real trading repositories — decides section by section what the implementer receives.",
                    "A section the classifier is unsure about is kept, and any section a kept section refers to is kept regardless of label.",
                    "Kept sections are verbatim; nothing is compressed or rewritten."],
            mustNot: ["that the classifier is a model-free control", "that the token saving has been proven before the builds run"]),
        S("cc.04", "classify-context", 35, AutoKind.Click, selector: "[data-auto='cc.claude']", bridge: true,
            wait: AutoWait.State, waitKey: "ClassifyFilter", timeout: 240, result: true,
            facts: ["Claude is asked to do the same filtering and is scored by the same rules.",
                    "The point of comparison is what an LLM filter costs on every turn versus a classifier that runs locally."]),
        S("cc.05", "classify-context", 35, AutoKind.Click, selector: "[data-auto='cc.implement']", bridge: true,
            wait: AutoWait.State, waitKey: "ClassifyContext", timeout: 900,
            facts: ["Gemma builds the bond settlement service from the full document, from the classifier's selection, and from Claude's selection.",
                    "Each build is compiled and run against a reference implementation; no model grades the result."]),
        S("cc.06", "classify-context", 35, AutoKind.Scroll, selector: "[data-auto='cc.review']", result: true,
            facts: ["Tokens down with cases equal is a saving; a case the full build passes and a filtered build fails names the section the filter should not have dropped.",
                    "Claude reviews the builds against sealed criteria it sees only now; the review explains the test results, it does not replace them.",
                    "This is one run per arm on two clearly different domains; adjacent domains are the harder test still to run."],
            mustNot: ["that this proves the approach for the whole estate"]),

        // ---- 36 focus-ignore --------------------------------------------------------------
        S("fx.01", "focus-ignore", 36, AutoKind.Scroll, selector: "[data-auto='fx.setup']",
            facts: ["The same mixed design document as the Classify Context scene, with the same sealed labels, traps and reference oracle.",
                    "The only thing that differs between the two scenes is how the off-topic sections are handled: removed there, named here."]),
        S("fx.02", "focus-ignore", 36, AutoKind.Click, selector: "[data-auto='fx.classify']",
            wait: AutoWait.Element, waitKey: "[data-auto='fx.classify-result']", timeout: 60, result: true,
            facts: ["The same local classifier runs unchanged; its dropped set becomes the list of sections the model will be told to ignore.",
                    "Uncertain sections and referenced sections stay in the read-in-full set, so the traps are protected the same way."]),
        S("fx.03", "focus-ignore", 36, AutoKind.Scroll, selector: "[data-auto='fx.instruction']",
            facts: ["The Focus/Ignore instruction is written by code from the classifier's table: section numbers and titles, no summaries, no reasons the classifier does not have.",
                    "The instructed prompt is the largest of the three by construction; it can only win on output tokens or correctness."],
            mustNot: ["that a model wrote the instruction"]),
        S("fx.04", "focus-ignore", 36, AutoKind.Click, selector: "[data-auto='fx.implement']", bridge: true,
            wait: AutoWait.State, waitKey: "Focus", timeout: 900,
            facts: ["Claude Opus 5 builds the bond settlement service from the mixed document, from the mixed document plus the instruction, and from the filtered document.",
                    "Each build is compiled and executed against the reference; prompt and output tokens are counted."]),
        S("fx.05", "focus-ignore", 36, AutoKind.Scroll, selector: "[data-auto='fx.review']", result: true,
        ...
        // ---- 40 llm-in-the-room ------------------------------------------------------------
        S("rm.01", "llm-in-the-room", 40, AutoKind.Scroll, selector: "[data-auto='rm.setup']",
            facts: ["A team wants a model in the room that listens when someone presses the microphone, answers in a few sentences, asks a question back, and keeps the notes.",
                    "The whole loop runs on this laptop: whisper.cpp transcribes, Gemma 4 E2B answers through llama.cpp on the CPU, and the browser reads the answer aloud; nothing said in the room leaves the machine."],
            mustNot: ["that a cloud model is involved", "that the VS Code bridge is used"]),
        S("rm.02", "llm-in-the-room", 40, AutoKind.Click, selector: "[data-auto='rm.start']",
            wait: AutoWait.State, waitKey: "Room", timeout: 180, needsExec: true,
            facts: ["Starting the room launches two host processes and pre-fills the grounding, so the first question does not pay for it.",
                    "The topic is the Open Knowledge Format, and Gemma is told to answer only from the fixture's own explanation of it."]),
        S("rm.03", "llm-in-the-room", 40, AutoKind.Fill, selector: "[data-auto='rm.typed']", value: "In one breath, what is the Open Knowledge Format and what does stale_after mean?",
            facts: ["The autopilot types the question because it has no microphone; a person in the room would press the button and speak.",
                    "The prompt carries the cached grounding and the last few turns, capped at three sentences and one hundred and sixty tokens."]),
        S("rm.04", "llm-in-the-room", 40, AutoKind.Click, selector: "[data-auto='rm.send']",
            wait: AutoWait.State, waitKey: "Room", timeout: 120, result: true,
            facts: ["Tokens stream to the screen as they are generated, and each finished sentence is queued to the browser's speech engine.",
                    "Every answer ends with one question back to the team; that is the instruction, not an accident."]),
        S("rm.05", "llm-in-the-room", 40, AutoKind.Scroll, selector: "[data-auto='rm.latency']", result: true,
            facts: ["First word, whole answer, decode speed, answer length and cache hit are each a stopwatch or a llama.cpp counter, shown against a target for a room.",
                    "The speed comes from caching the grounding and capping the answer, not from a faster model; a red row is the honest result on this hardware."],
            mustNot: ["that speculative decoding with a draft model made this faster", "that the latency would be the same on other hardware"]),
        S("rm.06", "llm-in-the-room", 40, AutoKind.Scroll, selector: "[data-auto='rm.notes']",
            facts: ["Between questions Gemma rewrites the running notes from the verbatim transcript; pressing the microphone cancels that so it never delays an answer.",
                    "The notes carry generated by Gemma and verified by nobody until a person signs them off; the transcript is the record."],
            mustNot: ["that the notes are verified", "that the transcript was produced by a model"])
    ];
